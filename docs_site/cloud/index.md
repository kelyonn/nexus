...
- **LoadBalancer services actually provision a cloud load balancer.**
  `service.yaml.j2` sets `type: LoadBalancer` — on Minikube that needs
  `minikube tunnel`; on a real cloud cluster it provisions a real (billed)
  load balancer per app. Know that before deploying more than a couple of
  apps.
...
- **Ingress isn't something Nexus sets up.** There's no Ingress template —
  Grafana, and any app you deploy, are reachable via `LoadBalancer`/
  port-forward, not a domain name. That's a deliberate scope boundary
  (PRD §0), not an oversight.
